  ...
  return (
    <div className="space-y-6 relative">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <h1 className="text-2xl md:text-3xl font-bold tracking-tight text-stone-900">Gerenciamento de Estoque</h1>
        ...
        <CardHeader>
          <div className="space-y-4">
            <div className="relative">
              <Search className="absolute left-3 top-1/2 -transtone-y-1/2 text-stone-400 h-4 w-4" />
              <input 
                type="text" 
                placeholder="Buscar por nome, SKU..." 
                className="w-full pl-10 pr-4 py-2 rounded-md border border-stone-200 focus:outline-none focus:ring-2 focus:ring-amber-500 text-sm"
                value={searchTerm}
            ...
            {/* Expanded Filters */}
            {showFilters && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2 border-t border-stone-100 animate-in slide-in-from-top-2">
                <div>
                  <label className="block text-xs font-medium text-stone-500 mb-1">Categoria</label>
                  <div className="relative">
                    <select 
                      className="w-full appearance-none bg-stone-50 border border-stone-200 text-stone-700 py-2 px-3 rounded-md focus:outline-none focus:ring-2 focus:ring-amber-500 text-sm"
                    ...
                    <ChevronDown className="absolute right-3 top-1/2 -transtone-y-1/2 h-4 w-4 text-stone-400 pointer-events-none" />
                  </div>
                </div>
                <div>
              ...
              <table className="min-w-full text-sm text-left">
                <thead className="bg-stone-50 text-stone-500 font-medium">
                  <tr>
                    ...
                    {canManageStock && <th className="px-4 py-3 text-right">Preço Unit.</th>}
                  ...
                  {paginatedInventory.map(item => {
                    ...
                    return (
                      <tr 
                        ...
                        className={`transition-colors cursor-pointer ${selectedItemId === item.id ? 'bg-amber-50 border-l-4 border-amber-500' : 'hover:bg-stone-50'}`}
                        // Clicking the row sets it as selected. Clicking again deselects it.
                        onClick={() => setSelectedItemId(selectedItemId === item.id ? null : item.id)}
                      >
                        <td className="px-4 py-3 font-medium text-stone-600">{item.sku}</td>
                        <td className="px-4 py-3 font-medium text-stone-900">{item.name}</td>
                        <td className="px-4 py-3 text-stone-500 hidden md:table-cell">{item.category}</td>
                        {canManageStock && <td className="px-4 py-3 text-right text-stone-600">{formatCurrency(item.price)}</td>}
                        <td className="px-4 py-3 text-right font-bold">{item.current_qty}</td>
                        <td className="px-4 py-3 text-center text-stone-400 text-xs hidden md:table-cell">{item.unit}</td>
                        <td className="px-4 py-3 text-right text-stone-500 hidden md:table-cell">{item.min_qty}</td>
                        ...
                        <td className="px-4 py-3 text-right whitespace-nowrap">
                          <div className="flex justify-end items-center gap-2">
                            <Button 
                              size="sm" 
                              variant="outline" 
                              className="bg-orange-50 border-orange-200 text-orange-600 hover:bg-orange-100 hover:text-orange-700 hover:border-orange-300 transition-colors"
                              onClick={(e) => {
                                e.stopPropagation();
                            ...
                            <Button 
                              ...
                              title="Solicitar Reposição de Insumo"
                            >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 overflow-y-auto">
          <Card className="w-full max-w-lg animate-in fade-in zoom-in-95 duration-200 my-auto">
            <CardHeader className="relative flex flex-row items-center justify-between pb-2">
              ...
              <button onClick={handleCloseItemModal} className="absolute top-4 right-4 text-stone-400 hover:text-stone-600">
                <X size={20} />
              </button>
            </CardHeader>
            <CardContent>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="col-span-2">
                    ...
                    <input 
                      ...
                      className="w-full p-2 border border-stone-300 rounded-md focus:ring-2 focus:ring-amber-500 focus:outline-none"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-stone-700 mb-1 flex justify-between">
                      SKU
                      {!editingItem && (
                        <span className="text-[10px] text-amber-600 font-normal self-center flex items-center gap-1">
                    ...
                    <input 
                      readOnly
                      type="text" 
                      className="w-full p-2 border border-stone-200 bg-stone-100 text-stone-500 rounded-md cursor-not-allowed focus:outline-none"
                  ...
                  {canManageStock && (
                   ...
                   <div>
                    ...
                    <select 
                      className="w-full p-2 border border-stone-300 rounded-md focus:ring-2 focus:ring-amber-500 focus:outline-none bg-white"
                  ...
                  {/* Quantities */}
                  <div className="col-span-2 grid grid-cols-2 gap-4 pt-2 border-t border-stone-100 mt-2">
                    <div>
                      <label className="block text-sm font-medium text-stone-700 mb-1 flex items-center gap-1">
                        ...
                        {!editingItem && <Lock className="w-3 h-3 text-stone-400" />}
                      </label>
                      <input 
                        ...
                        type="number" 
                        className={`w-full p-2 border border-stone-200 bg-stone-100 text-stone-500 rounded-md focus:outline-none font-medium ${!editingItem ? 'cursor-not-allowed' : ''}`}
                    ...
                    <div>
                      ...
                      />
                      <p className="text-[10px] text-stone-400 mt-1">Padrão sugerido: 5</p>
                    </div>
                  </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <Card className="w-full max-w-md animate-in fade-in zoom-in-95 duration-200">
            <CardHeader className="relative flex flex-row items-center justify-between pb-2">
              ...
              <button onClick={() => setIsRequestModalOpen(false)} className="absolute top-4 right-4 text-stone-400 hover:text-stone-600">
                <X size={20} />
              </button>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                <div className="bg-stone-50 p-3 rounded-md border border-stone-100">
                  <p className="font-medium text-stone-900">{selectedItemForRequest.name}</p>
                  <p className="text-sm text-stone-500">SKU: {selectedItemForRequest.sku} | Atual: {selectedItemForRequest.current_qty} {selectedItemForRequest.unit}</p>
                ...
                <div>
                  <label className="block text-sm font-medium text-stone-700 mb-1">Quantidade ({selectedItemForRequest.unit})</label>
                  <input 
                    type="number" 
                    min="1"
                    className="w-full p-2 border border-stone-300 rounded-md focus:ring-2 focus:ring-amber-500 focus:outline-none"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <Card className="w-full max-w-md animate-in fade-in zoom-in-95 duration-200 border-t-4 border-t-orange-500">
            <CardHeader className="relative flex flex-row items-center justify-between pb-2">
              ...
              <button onClick={() => setIsWithdrawModalOpen(false)} className="absolute top-4 right-4 text-stone-400 hover:text-stone-600">
                <X size={20} />
              </button>
            </CardHeader>
            <CardContent>
              <form onSubmit={submitWithdraw} className="space-y-4">
                <div className="bg-orange-50 p-3 rounded-md border border-orange-100">
                  <p className="font-medium text-stone-900">{selectedItemForWithdraw.name}</p>
                  <p className="text-sm text-stone-600">SKU: {selectedItemForWithdraw.sku}</p>
                ...
                <div>
                  <label className="block text-sm font-medium text-stone-700 mb-1">Quantidade a Retirar</label>
                  <input 
                    required
                    type="number" 
                    min="1"
                    max={selectedItemForWithdraw.current_qty}
                    className="w-full p-2 border border-stone-300 rounded-md focus:ring-2 focus:ring-orange-500 focus:outline-none"
                    value={withdrawQty}
                ...
                <div>
                  ...
                  <textarea 
                    ...
                    className="w-full p-2 border border-stone-300 rounded-md focus:ring-2 focus:ring-orange-500 focus:outline-none text-sm resize-none"
