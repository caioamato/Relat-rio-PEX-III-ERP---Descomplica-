  ...
  return (
    <div className="space-y-6 relative">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-stone-900">Painel Administrativo</h1>
          <p className="text-stone-500">Gerenciamento de usuários e auditoria do sistema</p>
        </div>
      </div>
              ...
              <div className="flex flex-col md:flex-row items-center gap-3 w-full md:w-auto">
                <div className="relative w-full md:w-64">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-4 py-2 text-sm border border-stone-200 rounded-md focus:outline-none focus:ring-2 focus:ring-amber-500 bg-white"
                    value={searchTerm}
                ...
                <div className="flex items-center gap-2 w-full md:w-auto justify-between md:justify-end">
                  <div className="flex items-center gap-1 bg-stone-100 p-1 rounded-md">
                     <button 
                       ...
                       className={`px-3 py-1 text-xs font-medium rounded-sm transition-all ${viewMode === 'active' ? 'bg-white text-amber-700 shadow-sm' : 'text-stone-500 hover:text-stone-700'}`}
                     >
                       Ativos
                     </button>
                     <button 
                       onClick={() => setViewMode('inactive')}
                       className={`px-3 py-1 text-xs font-medium rounded-sm transition-all ${viewMode === 'inactive' ? 'bg-white text-amber-700 shadow-sm' : 'text-stone-500 hover:text-stone-700'}`}
                     >
                       Inativos
                     </button>
                  </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm text-left">
                  ...
                  <tbody className="divide-y divide-stone-100">
                    {filteredUsers.map(u => (
                      <tr key={u.id} className={`group hover:bg-stone-50 transition-colors ${!u.is_active ? 'opacity-60 bg-stone-50/50' : ''}`}>
                        <td className="px-4 py-3 font-medium text-stone-900">
                          <div className="flex items-center gap-2">
                            {u.name} 
                            {u.id === currentUser.id && <span className="text-xs text-stone-400 font-normal">(Você)</span>}
                        ...
                        <td className="px-4 py-3">
                          <select 
                            className={`bg-white border border-stone-200 text-stone-700 text-xs rounded-md p-1.5 focus:ring-2 focus:ring-amber-500 focus:outline-none ${!isMaster || !u.is_active ? 'opacity-50 cursor-not-allowed bg-stone-100' : ''}`}
                            value={u.role}
                              ...
                              {u.is_active ? (
                                <>
                                  <button
                                    ...
                                    className="text-stone-400 hover:text-orange-500 transition-colors p-2 rounded-full hover:bg-orange-50"
                                ...
                                <button
                                  ...
                                  className="text-stone-400 hover:text-green-600 hover:bg-green-50 p-2 rounded-full transition-colors"
        ...
        <div className="space-y-6">
           <Card className="h-full max-h-[600px] flex flex-col">
            ...
            <CardContent className="flex-1 overflow-y-auto p-0 flex flex-col">
                 ...
                 {paginatedLogs.map(log => (
                   <div key={log.id} className="p-4 hover:bg-stone-50 transition-colors">
                     <div className="flex items-start justify-between mb-1">
                        <span className="font-semibold text-sm text-stone-800">{log.action}</span>
                        <span className="text-[10px] text-stone-400 whitespace-nowrap">
                          {new Date(log.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                        </span>
                     </div>
                     <p className="text-xs text-stone-600 mb-2">{log.description}</p>
                     
                     {log.previous_status && (
                       <div className="mb-2 text-[10px] bg-stone-100 text-stone-600 px-2 py-1 rounded inline-block">
               ...
               {logs.length > 0 && (
                  <div className="p-4 border-t border-stone-100 flex items-center justify-between bg-white shrink-0">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <Card className="w-full max-w-md animate-in fade-in zoom-in-95 duration-200">
            <CardHeader className="relative flex flex-row items-center justify-between pb-2">
              ...
              <button onClick={() => setIsAddUserModalOpen(false)} className="absolute top-4 right-4 text-stone-400 hover:text-stone-600">
                <X size={20} />
              </button>
            </CardHeader>
            <CardContent>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <Card className="w-full max-w-sm animate-in fade-in zoom-in-95 duration-200 border-t-4 border-t-orange-500">
            <CardHeader className="relative flex flex-row items-center justify-between pb-2">
              ...
              <button onClick={() => setUserToReset(null)} className="absolute top-4 right-4 text-stone-400 hover:text-stone-600">
                <X size={20} />
              </button>
            </CardHeader>
            <CardContent>
               <div className="space-y-4">
                 <p className="text-sm text-stone-600">
                   Deseja resetar a senha do usuário <span className="font-bold text-stone-900">{userToReset.name}</span> para o padrão?
                 </p>
                 <div className="bg-stone-100 p-2 rounded text-xs font-mono text-center border border-stone-200 text-stone-700">
