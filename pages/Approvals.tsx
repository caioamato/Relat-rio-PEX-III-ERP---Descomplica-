  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-stone-900">Central de Aprovações</h1>
        ...
        {pendingRequests.length > 0 && (
          <div className="flex items-center gap-3 bg-white p-2 rounded-md border border-stone-200 shadow-sm animate-in fade-in">
             <div className="flex items-center gap-2 px-2 border-r border-stone-200 pr-4">
                <input 
                  type="checkbox" 
                  ...
                  id="select-all"
                />
                <label htmlFor="select-all" className="text-sm font-medium text-stone-700 cursor-pointer select-none">
                  Selecionar Todos
                </label>
             </div>
      ...
      <div className="grid gap-4">
          ...
          return (
            <Card 
              ...
              className={`
                ...
                ${isNewItemRequest ? 'border-l-purple-500' : 'border-l-amber-500'}
                ${isSelected ? 'ring-2 ring-amber-500 bg-amber-50/30' : ''}
              `}
            >
              <CardContent className="p-0">
                <div className="flex">
                  ...
                  {req.status === 'PENDENTE' && (
                    <div className="flex items-center justify-center w-12 border-r border-stone-100 bg-stone-50/50 rounded-l-lg shrink-0" onClick={() => toggleSelection(req.id)}>
                      <input 
                        type="checkbox" 
                        checked={isSelected}
                    ...
                    <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                      ...
                      <div className="flex-1 space-y-2">
                        ...
                        <div className="flex flex-wrap gap-4 text-sm text-stone-500">
                          <span className="flex items-center gap-1">
                            <User className="w-3 h-3" /> Solicitado por: <span className="font-medium text-stone-700">{req.requester_name}</span>
                          </span>
                          <span className="flex items-center gap-1">
                          ...
                          {/* Feature: Stock Context Highlighting */}
                           {req.itemDetails ? (
                            <span className={`flex items-center gap-1 ${isLowStock ? 'text-red-600 font-bold bg-red-50 px-1 rounded' : 'text-stone-400'}`}>
                        ...
                        <div className="flex items-center gap-4 py-1">
                            <div className="text-xs bg-stone-50 px-2 py-1 rounded border border-stone-100 text-stone-600">
                                Valor Unit: <span className="font-medium text-stone-900">{formatCurrency(unitPrice)}</span>
                            </div>
                            <div className="text-xs bg-stone-50 px-2 py-1 rounded border border-stone-100 text-stone-600">
                                Valor Total: <span className="font-bold text-stone-900">{formatCurrency(totalValue)}</span>
                            </div>
                        </div>

                        {req.observation && (
                          <div className="mt-2 p-2 bg-stone-50 border border-stone-100 rounded text-sm text-stone-700 flex gap-2 items-start max-w-2xl">
                      ...
                      <div className="flex items-center gap-2 self-end md:self-center mt-2 md:mt-0">
                        ...
                        {(req.status === 'REJEITADO' || req.status === 'COMPRADO') && (
                           <span className="text-sm text-stone-400 italic px-3 border border-stone-100 rounded-md bg-stone-50">
        ...
        {enrichedRequests.length === 0 && (
          <div className="text-center py-10 text-stone-500 bg-white rounded-lg border border-dashed border-stone-300">
